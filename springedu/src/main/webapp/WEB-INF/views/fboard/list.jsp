...
<script>
	...
	function init(){
		...
		searchBtn.addEventListener("click",function(e){
			...
			location.href = getContextPath() + "/fboard/list/1/" + stype + "/" + kword;
		},false);
	}
						...
						<div id="boardList">
							...
							<div>
								<table class="login-wrapper">
									...
									<fmt:formatDate value="${rec.fcdate }" pattern="yyyy/MM/dd HH:mm" var="fcdate"/>						
									<tr>
										<td>${rec.bnum }</td>
										...
										<td  id="nottd0">
											...
											<%-- </c:if> --%>
											<a href="${pageContext.request.contextPath }/fboard/view/${pc.rc.reqPage}/${rec.fnum }">
												${rec.ftitle } 
											</a>
											
										</td>
										<td>${rec.fbnickname }</td>
										<td>${fcdate }</td>
									</tr>
								</c:forEach>
									<tr id="not-tr">
										<td colspan="5" id="nottd"></td>
										<td id="nottd"><button id="writeBtn" data-returnPage="${pc.rc.reqPage }">상품등록</button></td>
									</tr>
								</table>
							</div>
							<div id="paging">	
								<!-- 처음페이지 / 이전페이지 이동 -->
								<c:if test="${pc.prev }" >
									<a href="${pageContext.request.contextPath }/fboard/list/1/${pc.fc.searchType}/${pc.fc.keyword}"><i class="fas fa-angle-double-left"></i></a>
									<a href="${pageContext.request.contextPath }/fboard/list/${pc.startPage-1}/${pc.fc.searchType}/${pc.fc.keyword}"><i class="fas fa-angle-left"></i></a>
								</c:if>
								<c:forEach var="pageNum" begin="${pc.startPage }" end="${pc.endPage }">
									<!-- 현재페이지와 요청페이지가 다르면 -->
									<c:if test="${pc.rc.reqPage != pageNum }">
									<a href="${pageContext.request.contextPath }/fboard/list/${pageNum }/${pc.fc.searchType}/${pc.fc.keyword}" class="off">${pageNum }</a>
									</c:if>								
									<!-- 현재페이지와 요청페이지가 같으면 -->
									<c:if test="${pc.rc.reqPage == pageNum }">
									<a href="${pageContext.request.contextPath }/fboard/list/${pageNum }/${pc.fc.searchType}/${pc.fc.keyword}" class="on">${pageNum }</a>
									</c:if>
								</c:forEach>
								<!-- 다음페이지 / 최종페이지 이동 -->	
								<c:if test="${pc.next }" >	
									<a href="${pageContext.request.contextPath }/fboard/list/${pc.endPage+1}/${pc.fc.searchType}/${pc.fc.keyword}"><i class="fas fa-angle-right"></i></a>
									<a href="${pageContext.request.contextPath }/fboard/list/${pc.finalEndPage}/${pc.fc.searchType}/${pc.fc.keyword}"><i class="fas fa-angle-double-right"></i></a>
								</c:if>	
							</div>
							<div id="search">
								<form>
									<select name="searchType" id="searchType">
										<option value="TC" 
											<c:out value="${pc.fc.searchType == 'TC' ? 'selected':'' }" />>상품명+태그</option>
										<option value="T"
											<c:out value="${pc.fc.searchType == 'T' ? 'selected':'' }" />>상품명</option>
										<option value="C"
											<c:out value="${pc.fc.searchType == 'C' ? 'selected':'' }" />>제품코드</option>
										<option value="N"
											<c:out value="${pc.fc.searchType == 'N' ? 'selected':'' }" />>가격</option>
										<option value="I"
											<c:out value="${pc.fc.searchType == 'I' ? 'selected':'' }" />>제조사</option>
									</select>
